...
This project was made from the **blog** template, so it does not start empty: there is
one model, one resource, two block types and a page already published, and every
sentence below points at a file you can open. It is a worked example rather than a
content model anybody has to keep — see [Deleting the example](#deleting-the-example).
`--template bare` is the same project with none of it.
...
An article is almost certainly not what you are building, and none of it is load
bearing. What to remove, and what has to stop mentioning it:

- `src/models/article.ts` and `src/resources/articles.ts`, and the `.models()` and
  `.resources()` calls naming them in `src/modules/content.ts`. Keep the module — it
  is where your own declarations get registered.
<!-- assemora:if pages -->
- `src/blocks/` and `app/blocks/`, the `blocks: [Hero, RichText]` in `src/app.ts`, and
  the two entries in the registry in `app/main.tsx`. `pages()` with no blocks is a page
  builder with an empty palette, which is what a project has before anybody declares
  one.
<!-- assemora:end -->
- The body of `seed()` in `src/seed.ts`, down to the administrator. The seeded page and
  article live only in the throwaway in-memory database, so nothing has to be migrated
  to be rid of them.

If you have already run `pnpm db:migrate` against a real PostgreSQL, `pnpm db:generate`
after the first of those writes the migration that drops the table.

Then keep going with `assemora make:model`, `assemora make:resource` and
`assemora make:block` — the same three files, named after what you are actually
building.
