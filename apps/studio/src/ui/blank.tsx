...
export const NoCollections = ({
  ...
  >
    <p>
      A collection is a kind of content this application holds — Testimonials, Recipes, Team
      members. You give it a name and say what fields one entry has.
    </p>
    <p>
      {canCreate
        ? 'It is then a resource like every other: its own screen in the sidebar, the same policies, revisions and audit as everything else, and a tool an agent can call by name over MCP.'
        : 'Making one needs a permission this account does not have.'}
    </p>
  </Empty>
)
...
export const NoPages = ({ filtered, onCreate }: { filtered: boolean; onCreate(): void }) =>
  filtered ? (
    ...
    <Empty title="Make your first page" action={<Button onClick={onCreate}>New page</Button>}>
      ...
      <p>
        You put blocks on it in the builder, and publishing decides what a visitor sees. Until then
        it is a draft only you can open.
      </p>
    </Empty>
  )
...
export const NoEntries = ({
  ...
  >
    ...
    {editable && (
      <p>
        Nothing is stored against them yet, which makes this the cheapest moment to change what they
        are: what a value <em>is</em> becomes fixed as soon as one exists.
      </p>
    )}
  </Empty>
)
...
export const GettingStarted = ({
  ...
  <div className={`grid gap-3 ${canCreateCollection ? 'sm:grid-cols-3' : 'sm:grid-cols-2'}`}>
    {canCreateCollection && (
      <Step
        title="Describe some content"
        body="A collection is a kind of content named and shaped here rather than in TypeScript. Once it exists it is a resource like any other — a screen, the same policies, and a tool an agent can call."
        action={
    ...
    <Step
      title="Build a page"
      body="A page is a tree of blocks at an address. The builder puts blocks on it; publishing decides what a visitor sees."
      action={
    ...
    <Step
      title="Declare a block"
      body="Blocks are what a page is built from, and each one is a TypeScript declaration — so this is the step Studio cannot do for you."
